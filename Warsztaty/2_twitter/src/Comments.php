<?php
    ...
    class Comments {
        ...
        public static function loadCommentsByTweetId($conn, $id) {
            ...
            $sql = "SELECT * FROM comments WHERE tweet_id = $safeId";
        ...
        public static function displayCommentsOnPage($comments, $conn) {
        ...
        public function saveCommentToDB($conn) {
            ...
            $safeUserId = $conn->real_escape_string($this->userid);
            $sql = 'INSERT INTO comments ( comm, user_id, tweet_id) '
                    . 'VALUES("' . $safeText . '", ' . $safeUserId . ',' . $safeTweet . ' )';
            $result = $conn->query($sql);
            if ($result) {
                Print'<div class="form" style="margin-top : 5px">';
                Print 'dodałeś komentarz o treści  ' . $this->getText() . '<br>';
                Print"</div>";
            } else {
