<?php
    ...
    class Tweet {
        ...
        public static function loadTweets($conn, $id, $type) {
        ...
        public static function displayTweetsOnPage($tweets, $conn) {
            if (is_array($tweets)) {
                foreach ($tweets as $tweet) {
                    ...
                    if ($result != FALSE && $result->num_rows == 1) {
                        ...
                        print "ilość komentarzy do tego tweeta to <br>" . $counter
                                . "</div>";
                    }
                }
            } else {
        ...
        public static function displayTweetsOnTweetPage($tweets, $conn) {
            if (is_array($tweets)) {
                $tweet = $tweets[0];
                print'<div style="border:2px solid black">';
                Print 'ćwierknięcie z dnia' . $tweet->creationDate . '<br>';
                ...
                if ($result != FALSE && $result->num_rows == 1) {
                    while ($row = $result->fetch_assoc()) {
                        ...
                        <input type="hidden" name="TweetCreator" value="' . $tweet->userid . '">
                        <button type="submit" name="createMessage" value=""action="#">
                        Wyślij wiadomość do właściciela</button></form> ';
                    }
                    $comments = comments:: loadCommentsByTweetId($conn, $tweet->id);
                    Comments::displayCommentsOnPage($comments, $conn);
                    print'<form method="POST">
